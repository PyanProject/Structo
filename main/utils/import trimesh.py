...
def generate_3d_scene_from_embedding(generated_data, text, output_dir="models/generated"):
    ...
    # Если текст содержит "cube" или "куб", генерируем куб
    if "cube" in text.lower() or "куб" in text.lower():
        print("[MESH GEN] Detected 'cube' or 'куб' in text. Generating cube mesh.")
        try:
            # Параметры куба можно настроить – здесь создаётся куб единичного размера
            mesh = o3d.geometry.TriangleMesh.create_box(width=1.0, height=1.0, depth=1.0)
            mesh.compute_vertex_normals()
            o3d.io.write_triangle_mesh(scene_filename, mesh)
            print(f"[MESH GEN] Cube mesh saved: {scene_filename}")
            return scene_filename
        except Exception as e:
            print(f"[MESH GEN] Error generating cube mesh: {e}")
            return None

    # Дополнительная отладка: вывод формы и статистики сгенерированных данных
    try:
        print(f"[MESH GEN] Generated data shape: {generated_data.shape}")
        print(f"[MESH GEN] Data stats: min={np.min(generated_data)}, max={np.max(generated_data)}, mean={np.mean(generated_data)}")
    except Exception as e:
        print(f"[MESH GEN] Error computing stats for generated data: {e}")
        return None
