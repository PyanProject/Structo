...
def generate(args, device, need_visualisation=True):
    ...
    if not (v_min <= 0.5 <= v_max):
        level = (v_min + v_max) / 2
        print(f"Warning: level 0.5 not in voxel range [{v_min:.3f}, {v_max:.3f}]. Using level {level:.3f}")
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="datasets/testdataset", help="Path to dataset dir with OFF files")
