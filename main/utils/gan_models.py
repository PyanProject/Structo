...
def chamfer_distance(pc1, pc2):
    ...
    min1, _ = torch.min(dist, dim=2)
    min2, _ = torch.min(dist, dim=1)
...
class TNet(nn.Module):
    ...
    def forward(self, x):
        ...
        matrix = self.fc_layers(x)
        matrix = matrix.view(batch_size, self.in_dim, self.in_dim)
        return matrix
...
class Generator(nn.Module):
    def __init__(self, noise_dim=100, embedding_dim=512):
        ...
        self.fc = nn.Linear(noise_dim + embedding_dim, 3 * 4096)
