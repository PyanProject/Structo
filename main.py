...
class TestVoxelDataset(Dataset):
    """
    Загружает .off файлы из указанной директории и преобразует их в воксельное представление.
    Также генерирует текстовый prompt на основе имени файла.
    """
    ...
    def __getitem__(self, idx):
        ...
        # Получаем матрицу вокселей и центруем её в фиксированном тензоре
        voxels = voxel_obj.matrix.astype(np.float32)
        voxel_tensor = self._center_voxel_grid(voxels)

        # Генерируем prompt на основе имени файла
    ...
    def _voxelize_mesh(self, mesh):
        ...
        try:
            ...
            try:
                ...
                print(f"Warning: Skipping file during voxelization: {e}")
                return None
        return voxel_obj
...
class VoxelEncoder(nn.Module):
    """
    3D Convolutional Encoder, который преобразует входной воксельный тензор в латентное представление.
    """
...
class ConditionalVoxelDecoder(nn.Module):
    """
    Декодер, который принимает латентное представление и условный вектор (например, текстовый embedding),
    а затем восстанавливает воксельную сетку.
    """
...
class CVAE_Conditional(nn.Module):
    """
    Комбинированная модель Conditional VAE, включающая энкодер и декодер.
    """
...
class Discriminator(nn.Module):
    """
    Дискриминатор для 3D воксельных сеток.
    """
    def __init__(self, voxel_size):
        ...
        self.relu = nn.ReLU()
        final_dim = voxel_size // 16  # предполагается, что voxel_size кратен 16
...
def vae_loss(recon, x, mu, logvar):
    """
    Комбинированная функция потерь для VAE: сумма бинарной кросс-энтропии и KL-дивергенции.
    """
...
def encode_prompt(prompt, device):
    """
    Кодирует текстовый prompt с использованием модели CLIP.
    """
...
def compute_iou(recon, target, threshold=0.5):
    """
    Вычисляет IoU между бинарными воксельными сетками.
    """
...
def collate_fn_skip_none(batch):
    """
    Фильтрует элементы батча, где первый элемент (voxel_tensor) равен None.
    """
...
def train(args, device):
    ...
    # Загрузка модели CLIP для текстового кодирования
    model_clip, _ = clip.load("ViT-B/32", device=device)
    model_clip.eval()
    ...
    for epoch in range(args.epochs):
        ...
        for voxels, prompts in dataloader:
            ...
            # Обучение генератора (2 обновления)
            total_gen_loss = 0.0
            for _ in range(2):
...
def generate(args, device, need_visualisation=True):
    ...
    # Определяем порог для marching cubes
...
def main():
    ...
    # Всегда используем заданный checkpoint
